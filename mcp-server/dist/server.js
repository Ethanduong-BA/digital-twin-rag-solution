...
// Tool Definition
// ============================================
const compareProfileWithJobTool = {
    name: "compare_profile_with_job",
    description: "Compare your profile against a job description. Analyzes skills match, identifies gaps, and provides a compatibility score (1-10).",
    inputSchema: {
        type: "object",
        properties: {
            job_filename: {
                type: "string",
                description: "Name of the job file in jobs/ folder (e.g., 'week3-job01-the-star-entertainment-group-data-analyst.md')",
            },
        },
...
function readJobFile(filename) {
    try {
        ...
        console.error(`[ERROR] readJobFile:`, error);
        throw new Error(`Failed to read job file "${filename}": ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
...
function readUserProfile() {
    try {
        const mcpServerDir = path.dirname(__dirname); // /mcp-server
        const projectRoot = path.dirname(mcpServerDir); // /digital-twin-rag-solution
        const profilePath = path.join(projectRoot, "data", "my-profile.md");
        // Uncomment for debugging
        ...
        const jsonProfilePath = path.join(projectRoot, "data-pipeline", "raw_data", "profile.json");
        ...
        throw new Error("Profile not found at data/my-profile.md or data-pipeline/raw_data/profile.json");
    }
    catch (error) {
        console.error(`[ERROR] readUserProfile:`, error);
        throw new Error(`Failed to read profile: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 * Convert JSON profile to text format for analysis
 */
function convertProfileToText(profile) {
    try {
        let text = "";
        // Basic info
        ...
        if (profile.skills && Array.isArray(profile.skills)) {
            text += "Skills: ";
            text += profile.skills.map((s) => typeof s === 'string' ? s : s.name).join(", ");
            text += "\n\n";
        }
        // Experience
        if (profile.experience && Array.isArray(profile.experience)) {
            ...
            profile.experience.forEach((exp) => {
                ...
                if (exp.skills)
                    text += `  Skills: ${Array.isArray(exp.skills) ? exp.skills.join(", ") : exp.skills}\n`;
            });
            text += "\n";
        }
        // Education
        ...
        if (profile.projects && Array.isArray(profile.projects)) {
            ...
            profile.projects.forEach((proj) => {
                ...
                if (proj.skills)
                    text += `  Skills: ${Array.isArray(proj.skills) ? proj.skills.join(", ") : proj.skills}\n`;
            });
            text += "\n";
        }
        // Certifications
        if (profile.certifications && Array.isArray(profile.certifications)) {
            text += "Certifications: ";
            text += profile.certifications.map((c) => typeof c === 'string' ? c : c.name).join(", ");
            text += "\n";
        }
 ...
 */
function analyzeProfileJobMatch(profile, jobContent, jobTitle, company) {
    ...
    const skillKeywords = {
        ...
        "Power BI/Tableau": { keywords: ["power bi", "tableau", "looker", "qlik"], weight: 3 },
    ...
    Object.entries(skillKeywords).forEach(([skill, config]) => {
        const hasSkillInProfile = config.keywords.some((kw) => profileLower.includes(kw));
        const requiresSkillInJob = config.keywords.some((kw) => jobLower.includes(kw));
    ...
    const gapPoints = missingSkills.map((skill) => ({
        skill,
    ...
    const overallScore = Math.max(1, Math.min(10, Math.round((matchPercentage / 100) * 8 + 2)));
    // Generate recommendation
    const recommendation = generateRecommendation(overallScore, matchPoints, gapPoints, jobTitle);
    return {
        jobTitle,
        company,
        matchPoints,
 ...
 */
function determineProficiency(skill, profileText) {
    ...
    const skillContext = profileText.substring(Math.max(0, profileText.indexOf(skillLower) - 100), profileText.indexOf(skillLower) + 100);
 ...
 */
function determineImportance(skill, jobText) {
    const skillLower = skill.toLowerCase();
    // Look for importance indicators in job description
    const requiredPattern = /required|must have|essential|mandatory|critical/i;
    const preferredPattern = /preferred|nice to have|beneficial|desirable/i;
    const skillContext = jobText.substring(Math.max(0, jobText.indexOf(skillLower) - 150), jobText.indexOf(skillLower) + 150);
 ...
 */
function generateRecommendation(score, matches, gaps, jobTitle) {
    if (score >= 9) {
...
// Main Tool Handler
// ============================================
export async function handleCompareProfileWithJob(jobFilename) {
    try {
        // Read job file
        const { title, company, content: jobContent } = readJobFile(jobFilename);
        // Read user profile
        const profileContent = readUserProfile();
        // Perform analysis
        const result = analyzeProfileJobMatch(profileContent, jobContent, title, company);
        return result;
    }
    catch (error) {
        ...
        throw new Error(`Failed to compare profile with job: ${errorMsg}`);
    }
}
// ============================================
// Export for MCP Server Integration
// ============================================
export { compareProfileWithJobTool };
